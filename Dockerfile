...
ENV MONERO_VERSION=0.17.1.5

WORKDIR /root
RUN apt-get -y update && apt-get -y install curl lbzip2 && \
    curl -Os https://gui.xmr.pm/files/block.txt && \
...
COPY --from=build --chown=monero:monero /root/block.txt /etc/block.txt

USER monero
EXPOSE 18080 18081

ENTRYPOINT ["monerod"]
CMD ["--rpc-bind-ip=0.0.0.0", "--data-dir=/monero", "--rpc-ssl=enabled", "--restricted-rpc", "--public-node",  "--confirm-external-bind",  "--non-interactive", "--no-zmq", "--enforce-dns-checkpointing", "--log-level=1", "--ban-list=/etc/block.txt"]
